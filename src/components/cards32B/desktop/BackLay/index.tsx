...
const BackLay = ({ matchOddsData, data }: any) => {
  ...
  const renderItem = (item: any, index: number, type: any) =>
    type === "back" ? (
      <div
        className={`dtlsubTitle ${type}-BackGround ${handleLock(item?.gstatus, item?.b1) ? "lock-32" : ""
          } `}
        onClick={() =>
      ...
      <div
        className={`dtlsubTitle ${type}-BackGround ${handleLock(item?.gstatus, item?.l1) ? "lock-32" : ""
          } `}
        onClick={() =>
  ...
  return (
    <div className="w-100">
      ...
      >
        ...
        <div className={isMobile ? "row-flex-mobile" : "w-100 d-sm-flex flex-row"}>
          <span className="dtlTitle lh-1">
            <div className="profitLoss-Text">
